function [error] = RBFL2Error_const(x_array, y_array, numerical_solu, target_func, phi)   % calcalate L^2 error
% x_array = [0, 0.2, 0.4, ..., 3]
% y_array = [-1, -0.5, ..., 1]
...
for i = 1:N1-1
    for j = 1:N2-1
        ...
        mat = [phi(0,0),phi(h2,0),phi(h1,0),phi(sqrt(h1.*h1+h2.*h2),0),1;phi(h2,0),phi(0,0),phi(sqrt(h1.*h1+h2.*h2),0),phi(h1,0),1; ...
               phi(h1,0),phi(sqrt(h1.*h1+h2.*h2),0),phi(0,0),phi(h2,0),1;phi(sqrt(h1.*h1+h2.*h2),0),phi(h1,0),phi(h2,0),phi(0,0),1;...
               1,1,1,1,0];
        lambda = mat\([u1,u2,u3,u4,0]');
        func = @(x,y)( lambda(1).*phi( x-x_array(i),y-y_array(j) )+lambda(2).*phi( x-x_array(i),y-y_array(j+1) ) ...
                      +lambda(3).*phi( x-x_array(i+1),y-y_array(j) )+lambda(4).*phi( x-x_array(i+1),y-y_array(j+1) ) + lambda(5) );
