...
for i = 1:N1-1
    for j = 1:N2-1
        ...
        func = @(x,y)( u1.*(1-(x-x_array(i))./(h1)).*(1-(y-y_array(j))./(h2)) + u2.*(1-(x-x_array(i))./(h1)).*(1-(y_array(j+1)-y)./(h2)) ...
                      + u3.*(1-(x_array(i+1)-x)./(h1)).*(1-(y-y_array(j))./(h2)) + u4.*(1-(x_array(i+1)-x)./(h1)).*(1-(y_array(j+1)-y)./(h2)) );
