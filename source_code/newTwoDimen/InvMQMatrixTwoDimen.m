...
function [result] = InvMQMatrixTwoDimen(x_start, y_start, i_x, i_y, j_x, j_y, h1,h2,x_num)
...
c = 1.2;
func_dx = @(x,y)( -c.*c.*x./sqrt( (1+c.*c.*(x.*x+y.*y)).^3 ) );
func_dy = @(x,y)( -c.*c.*y./sqrt( (1+c.*c.*(x.*x+y.*y)).^3 ) );
