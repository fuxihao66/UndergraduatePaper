function [result] = MQVectorTwoDimen(x_start, y_start,i,h1,h2,f,x_num,y_num,boundary)
c = 0.81;
fu = @(x,y)( sqrt(1+c.*c.*(x.^2+y.^2)) );
phi = @(r)( sqrt(1+c.*c.*r.*r) );
numNodesX = x_num - 1;
numNodesY = y_num - 1;
...
if i_x == 1 || i_y == 1 || i_x == numNodesX || i_y == numNodesY
    for yindex=0:y_num
    % x = 0 / x_num
        result = result - MQMatrixTwoDimen(x_start, y_start, i_x, i_y, 0, yindex, h1,h2,x_num)*boundary(x_start,y_start+yindex*h2);           %(x_start, y_start+yindex*h2)
        result = result - MQMatrixTwoDimen(x_start, y_start, i_x, i_y, x_num, yindex, h1,h2,x_num)*boundary(x_start+x_num*h1,y_start+yindex*h2);  %(x_end, y_start+yindex*h2)
    end
    for xindex=1:numNodesX       
        result = result - MQMatrixTwoDimen(x_start, y_start, i_x, i_y, xindex, 0, h1,h2,x_num)* boundary(x_start+xindex*h1 ,y_start);            %(x_start+xindex*h1, y_start)
        result = result - MQMatrixTwoDimen(x_start, y_start, i_x, i_y, xindex, y_num, h1,h2,x_num)* boundary(x_start+xindex*h1 ,y_start+y_num*h2);   %(x_start+xindex*h1, y_end)
    end
end
