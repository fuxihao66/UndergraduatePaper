function [result] = getMQMatrixEle_poly(i,j,x_0,h, p, q, scale, epsilon)


pphi = @(x,c)(sqrt(1+c.*x.*x));
pphi_d = @(x,c)(c.*x./sqrt(1+c.*x.*x) );
...
if mod(i,2)==0 && mod(j,2)==0  %% both are node
    ...
    elseif i == j && i == scale*2
        ...
        phi_d = @(x)(pphi_d(x, epsilon(scale)));
    ...
    elseif abs(i-j) == 2
        ...
        phi = @(x)(pphi(x, epsilon(index/2+1)));
        phi_d = @(x)(pphi_d(x, epsilon(index/2+1)));
        ...
        phi_d = @(x)(pphi_d(x, epsilon(i/2+1)));
...
elseif mod(i,2)==1 && mod(j,2)==1 %% both are mid
    if i == j
        ...
        phi_d = @(x)(pphi_d(x, epsilon((i+1)/2)));
    ...
    elseif i - j == 1
        result = getMQMatrixEle_poly(j,i,x_0,h, p, q, scale, epsilon);                                     % remember to modifiy
    else
        result = 0;
    end
end